...
export const createTable = (columnsArray, dataArray, tableId) => {
  if (!isNonEmptyArray(columnsArray) || !isNonEmptyArray(dataArray) || !tableId) {
    throw new Error(
      "Para a correta execução precisamos de um  array com as colunas, outro com as informações das linhas e também o id da tabela selecionados"
    );
  }

  const tableElement = document.getElementById(tableId);
  if (!tableElement || tableElement.nodeName !== "TABLE") {
    throw new Error("Não foi possível encontrar a tabela com o id informado");
  }
